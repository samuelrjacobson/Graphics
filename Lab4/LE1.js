...
var shapes = {
    ...
    octahedron: { points: [], normals: [], start: 0, size: 0, type: 0 }
};
...
var octahedronLookups = [
    3, 0, 5,    // upper left front
    3, 5, 1,    // upper right front
    3, 4, 0,    // upper left back
    3, 1, 4,    // upper right back
    5, 0, 2,    // lower left front
    1, 5, 2,    // lower right front
    4, 1, 2,    // lower right back
    4, 2, 0,    // lower left back
];
...
for (var i = 0; i < octahedronLookups.length; i++) {
    shapes.octahedron.points.push(cubeVerts[octahedronLookups[i]]);
    shapes.octahedron.normals.push(normalsList[faceNum]);
    if (i % 8 == 7) faceNum++; //Switch color for every face. 6 vertices/8 faces
}*/
//Expand octahedron data:
//  Uses cross product as shown in lab demo.
for (var i = 0; i < octahedronLookups.length; i++) {
    shapes.octahedron.points.push(octahedronVerts[octahedronLookups[i]]);
    shapes.octahedron.normals.push(vec3(0, 0, 0));
}
makeFlatNormals(shapes.octahedron.points, //array full of vertices for gl.TRIANGLES
    0, //index of where those vertices start
    shapes.octahedron.points.length, //number of vertices
    shapes.octahedron.normals //destination array for normals
);
...
// Calculates Flat Normals for Triangles
// Input parameters:
...
window.onload = function init() {
    ...
    loadShape(shapes.octahedron, gl.TRIANGLES);
...
function render() {
    ...
    var cubeTF = mult(mv, cubeRot); // cube rotation
    ...
    gl.drawArrays(shapes.octahedron.type, shapes.octahedron.start, shapes.octahedron.size);
